...
**基于 Tauri v2 + React 打造的高性能跨平台 Android 设备管理工具**
...
**玩机管家 (ADMT)** 是一款面向 Android 开发者、刷机爱好者与普通机主的桌面端综合设备管理工具。底层采用 **Rust + Tauri v2** 原生架构，相比传统 Electron 框架应用具有极低的内存占用与快速的响应体验。

### 核心特性
- **设备管理与监控**：硬件参数、电池健康、RAM 与存储占用、CPU 频率与温度实时图表。
- **ADB 工具箱**：无线 ADB 快速配对、应用管理（安装/卸载/免 Root 冻结）、Scrcpy 低延迟投屏。
- **刷机与 Root 修补**：Fastboot 镜像烧录、Boot 镜像修补 (Magisk / KernelSU / APatch)、A/B 分区切换。
- **诊断与控制台**：ADB/Fastboot 命令终端、Logcat 实时日志过滤与崩溃排障。

---

## 架构与技术选型

项目采用前后端解耦的响应式架构：前端采用 **React 18 + Fluent UI v9**，跨进程通信通过 **Tauri v2 IPC** 直接驱动原生二进制（ADB、Fastboot、Scrcpy）及系统底层能力。
...
│ └──────────────┘ └──────────────┘ └──────────────┘ └──────────────┘ └──────────────┘ │
│ ┌──────────────┐ ┌──────────────┐ ┌──────────────┐ ┌──────────────┐ ┌──────────────┐ │
│ │ 极客拓展工具 │ │ 在线资源中心 │ │  诊断控制台  │ │ 全局偏好设置 │ │ 状态栏与通知 │ │
│ └──────────────┘ └──────────────┘ └──────────────┘ └──────────────┘ └──────────────┘ │
├──────────────────────────────────────────────────────────────────────────────────────────┤
...
**如果玩机管家 (ADMT) 对你的刷机、开发或设备管理有所帮助，欢迎在 GitHub 上为我们点一个 Star！**
