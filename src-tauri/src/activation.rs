...
impl ActivationValidator {
    ...
    pub async fn validate_code(&self, code: &str) -> Result<ActivationCode> {
        ...
        if !self.validate_format(code) {
            return Err(AdmtError::Tool("激活码格式不正确".to_string()));
        }
                ...
                if status.is_success() {
                    ...
                    match serde_json::from_str::<serde_json::Value>(&response_text) {
                                ...
                                Ok(verify_response) => {
                                    if verify_response.success {
                                        if let Some(verify_data) = verify_response.data {
                                            ...
                                            Err(AdmtError::Tool("API响应数据为空".to_string()))
                                        }
                                    } else {
                                        let error_msg =
                                            verify_response.error.unwrap_or("未知错误".to_string());
                                        Err(AdmtError::Tool(error_msg))
                                    }
                                }
                                Err(e) => {
                                    ...
                                    {
                                        Err(AdmtError::Tool(error_msg.to_string()))
                                    } else if let Some(message) =
                                        json_value.get("message").and_then(|v| v.as_str())
                                    {
                                        Err(AdmtError::Tool(message.to_string()))
                                    } else {
                                        Err(AdmtError::Tool(format!("API响应结构不匹配: {}", e)))
                                    }
                                }
                            }
                        }
                        Err(e) => {
                            ...
                            Err(AdmtError::Tool(format!("API响应不是有效的JSON格式: {}", e)))
                        }
                    }
                } else {
                    // 处理HTTP错误状态码
                    match status.as_u16() {
                        400 => {
                                ...
                                serde_json::from_str::<VerifyResponse>(&response_text)
                            {
                                ...
                                Err(AdmtError::Tool("激活码无效".to_string()))
                            }
                        }
                        404 => Err(AdmtError::Tool("激活码不存在".to_string())),
                        500 => Err(AdmtError::Tool("服务器内部错误，请稍后重试".to_string())),
                        _ => Err(AdmtError::Tool(format!("验证失败，HTTP状态码: {}", status))),
                    }
                }
            }
            Err(e) => {
                log::error!("Failed to call activation API: {}", e);
                Err(AdmtError::Network(format!("网络请求失败: {}", e)))
            }
        }
    }
