        ...
        if let Some(stripped) = line.strip_prefix("(bootloader) ") {
            if let Some((key, value)) = stripped.split_once(':') {
                ...
                match key {
                    ...
                    "slot-unbootable:a" => properties.slot_successful_a = parse_bool(value).map(|b| !b), // 可引导状态取反
                    "slot-unbootable:b" => properties.slot_successful_b = parse_bool(value).map(|b| !b), // 可引导状态取反
                    ...
                    // 存储与分区结构 - 并行刷写支持
                    "parallel-download-flash" => properties.parallel_download_flash = parse_bool(value),
                    
                    // 其他辅助参数
                    "off-mode-charge" => properties.off_mode_charge = parse_bool(value),
                    "charger-screen-enabled" => properties.charger_screen_enabled = parse_bool(value),
...
/// 获取fastboot设备属性
#[tauri::command]
pub async fn get_fastboot_device_properties(
    serial: String,
) -> Result<FastbootDeviceProperties> {
    ...
    log::info!("[fastboot_device_info] fastboot getvar all输出长度: {} 字符", result.output.len());
    
    // 解析输出
    let properties = parse_fastboot_getvar_all(&result.output);
    
    log::info!("[fastboot_device_info] 解析完成，找到 {} 个分区类型，{} 个分区大小", 
...
pub fn convert_to_device_properties(fastboot_props: &FastbootDeviceProperties) -> crate::device::DeviceProperties {
    ...
    device_props.anti_rollback = fastboot_props.anti.clone().and_then(|anti| parse_bool(&anti)); // 将anti字符串解析为布尔值
...
"#;
        ...
        assert_eq!(props.partition_types.get("boot_a"), Some(&"raw".to_string()));
        assert_eq!(props.partition_types.get("userdata"), Some(&"f2fs".to_string()));
        assert_eq!(props.partition_sizes.get("boot_a"), Some(&"0xC000000".to_string()));
        assert_eq!(props.partition_sizes.get("userdata"), Some(&"0x385B7FB000".to_string()));
    }
