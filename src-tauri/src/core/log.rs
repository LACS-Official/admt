...
use chrono::{Local, Utc, Duration, TimeZone}; 
...
pub fn get_app_data_dir() -> Result<PathBuf> { 
    ...
    if let Ok(current_dir) = std::env::current_dir() { 
        ...
        if let Err(err) = std::fs::create_dir_all(&app_dir) { 
            ...
            return Err(AdmtError::FileOperationFailed { message: format!("创建应用数据目录失败: {}", err) }); 
        } 
        return Ok(app_dir); 
    } 
    ...
    if let Err(err) = std::fs::create_dir_all(&default_path) { 
        ...
        return Err(AdmtError::FileOperationFailed { message: format!("创建默认应用数据目录失败: {}", err) }); 
    } 
    Ok(default_path) 
} 
...
pub fn initialize_log_directory() -> Result<PathBuf> { 
    ...
    if let Err(err) = fs::create_dir_all(&logs_dir) { 
        ...
        return Err(AdmtError::FileOperationFailed { message: format!("创建日志目录失败: {}", err) }); 
    } 
...
/// 持久化日志到文件（简化版本）
#[tauri::command]
pub fn persist_log(level: String, message: String, context: Option<serde_json::Value>) -> Result<()> {
                                ...
                                if let Ok(log_entry) = serde_json::from_str::<StructuredLogEntry>(line) { 
                                    ...
                                    if let Some(module_count) = modules.get_mut(&log_entry.module) { 
                                        if let serde_json::Value::Number(n) = module_count { 
                                            if let Some(new_count) = n.as_u64().map(|x| x + 1) { 
                                                *module_count = serde_json::Value::Number(new_count.into()); 
                                            } 
                                        } 
                                    } else { 
                                        modules.insert(log_entry.module.clone(), serde_json::Value::Number(1.into())); 
                                    } 
                                } 
                            } 
                        } 
...
/// 清理过期日志 
#[tauri::command]
pub fn cleanup_expired_logs(basic_cutoff_days: Option<u64>, error_cutoff_days: Option<u64>) -> Result<()> { 
    let logs_dir = initialize_log_directory()?; 
                        ...
                        if let Some(date_str) = filename.strip_prefix("admt_log_").and_then(|s| s.strip_suffix(".log")) { 
                            if let Ok(file_date) = chrono::NaiveDate::parse_from_str(date_str, "%Y-%m-%d") { 
                                ...
                                let should_cleanup = if date_str == now.format("%Y-%m-%d").to_string() { 
                                    ...
                                    // 根据是否包含错误决定清理期限 
                                    let cutoff = if contains_errors { error_cutoff } else { basic_cutoff }; 
                                    now.signed_duration_since(file_datetime) > cutoff 
                                }; 
                                
                                if should_cleanup { 
                                    if let Err(err) = fs::remove_file(&path) { 
                                        log::error!("Failed to delete expired log file {}: {}", path.display(), err); 
                                    } else { 
                ...
                if path.is_file() { 
                    ...
                    if filename.starts_with("admt_log_") && filename.ends_with(".log") { 
                        if let Err(err) = fs::remove_file(&path) { 
                            ...
                            return Err(AdmtError::FileOperationFailed { message: format!("删除日志文件失败: {}", err) }); 
                        } 
                    } 
                } 
            } 
