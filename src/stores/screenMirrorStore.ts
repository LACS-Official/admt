...
interface ScreenMirrorActions {
  // 设置所有活动会话（通常用于初始化同步）
  setActiveSessions: (sessions: ScreenMirrorSession[]) => void;
  ...
  updateActiveSession: (
    sessionId: string,
    updates: Partial<ScreenMirrorSession>,
  ) => void;

  // 获取指定设备的活动会话
  getActiveSessionByDevice: (
    deviceSerial: string,
  ) => ScreenMirrorSession | null;
...
export const useScreenMirrorStore = create<
  ScreenMirrorState & ScreenMirrorActions
>()(
  persist(
    (set, get) => ({
      // 初始状态
      ...
      addActiveSession: (session) =>
        set((state) => {
          // 防止重复添加相同 ID 的会话
          if (state.activeSessions.some((s) => s.id === session.id)) {
            return state;
          }
      ...
      updateActiveSession: (sessionId, updates) =>
        set((state) => ({
