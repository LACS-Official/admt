        ...
        <Card className={styles.displayCard}>
            <div className={styles.displayContainer}>
                {session.status === 'streaming' ? (
                    <div className={styles.mirrorPlaceholder}>
                        ...
                        <Text size={300} style={{ color: "var(--colorNeutralForeground2)" }}>{t('mirror.mirror_new_window')}</Text>
                        ...
                        <div className={styles.buttonGroup}>
                            <Button appearance="subtle" icon={<PhoneDesktop24Regular />} onClick={handleOpenScrcpyWindow} size="small">{t('mirror.find_window')}</Button>
                            <Button appearance="subtle" icon={<Screenshot24Regular />} onClick={handleTakeScreenshot} size="small">{t('mirror.screenshot')}</Button>
                            <Button appearance="subtle" icon={isRecording ? <RecordStop24Regular /> : <Record24Regular />} onClick={handleToggleRecording} size="small">
                                {isRecording ? t('mirror.stop_record') : t('mirror.record')}
                            </Button>
                            <Button appearance="subtle" icon={<Info24Regular />} onClick={() => setShowDetails(!showDetails)} size="small">
                        ...
                        {showDetails && (
                            <div className={styles.detailsCard}>
                                ...
                                <Text size={200}>{t('mirror.resolution_label')}: {session.config.quality?.resolution}</Text>
                            </div>
                        )}
                    </div>
                ) : (
            ...
            <div className={styles.controls}>
                <div className={styles.statusInfo}>
                    <Badge appearance="filled" color="success">{session.status === 'streaming' ? t('mirror.mirroring') : t('mirror.connecting')}</Badge>
                    <Text size={200}>{formatDuration(session.startTime)}</Text>
                </div>
                <div className={styles.controlsRight}>
                    {onStopMirror && <Button appearance="subtle" icon={<Stop24Regular />} onClick={onStopMirror}>{t('mirror.stop_mirror')}</Button>}
                    <Button appearance="subtle" icon={<FullScreenMaximize24Regular />} onClick={toggleFullscreen}>{t('mirror.fullscreen')}</Button>
                </div>
            </div>
        </Card>
    );
