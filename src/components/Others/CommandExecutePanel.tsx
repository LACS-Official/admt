...
const CommandExecutePanel : React.FC = () => {
  ...
  const getDeviceStatusColor = (device: DeviceInfo) => {
    ...
    switch (device.mode) {
      case 'sys': return '#4CAF50'; // 绿色
      case 'rec': return '#FF9800'; // 橙色
      case 'fastboot':
      case 'fastbootd': return '#2196F3'; // 蓝色
      case 'sideload': return '#9C27B0'; // 紫色
      case 'edl': return '#F44336'; // 红色
      case 'unauthorized': return '#FF5722'; // 深橙色
      case 'offline': return '#607D8B'; // 蓝灰色
  ...
  const filteredCommands = useMemo(() => {
    if (!commandsConfig) return [];
    return filterCommandsBySearchTerm(commandsConfig.categories, dialogSearchTerm);
  }, [commandsConfig, dialogSearchTerm]);
  
  // 获取统一面板过滤后的命令分类
  const unifiedPanelFilteredCommands = useMemo(() => {
    if (!commandsConfig) return [];
    return filterCommandsBySearchTerm(commandsConfig.categories, unifiedPanelSearchTerm);
...
const executeCommand = async (cmd: string) => {
  ...
  try {
    ...
    console.log(`执行${commandType}命令:`, { command: commandName, args, device: selectedDevice.serial });
    ...
    setOutput(prev => prev + `[${timestamp}] ${deviceInfo} 错误: ${errorMsg}\n\n`);
    // 显示命令执行失败提示
    setStatusBarMessage({
      type: 'error',
      message: errorMsg
  ...
  // 处理命令编辑
  const handleEditCommand = (command: AdbCommand, category: CommandCategory) => {
  ...
  // 处理命令删除
  const handleDeleteCommand = async (command: AdbCommand, category: CommandCategory) => {
  ...
  // 编辑命令相关函数
  const openEditCommandDialog = (command: AdbCommand, category: CommandCategory) => {
    setEditingCommand(command);
    setEditingCategory(category);
    setEditForm({
      id: command.id,
  ...
  const saveCommand = async () => {
    ...
    try {
      ...
      const categoryIndex = updatedConfig.categories.findIndex(cat => cat.id === editingCategory.id);
      ...
      if (isAddingNewCommand) {
        ...
        // 更新现有命令
        const commandIndex = updatedConfig.categories[categoryIndex].commands.findIndex(
          cmd => cmd.id === editingCommand?.id
        );
  ...
  const deleteCommand = async (command: AdbCommand, category: CommandCategory) => {
    if (!commandsConfig) return;
    
    if (confirm(`确定要删除命令"${command.label}"吗？`)) {
      try {
        const updatedConfig = { ...commandsConfig };
        const categoryIndex = updatedConfig.categories.findIndex(cat => cat.id === category.id);
        
        if (categoryIndex === -1) return;

        updatedConfig.categories[categoryIndex].commands = updatedConfig.categories[categoryIndex].commands.filter(
  ...
  const deleteCategory = async (category: CommandCategory) => {
    ...
    if (confirm(`确定要删除分类"${category.name}"及其所有命令吗？`)) {
      try {
        const updatedConfig = { ...commandsConfig };
        updatedConfig.categories = updatedConfig.categories.filter(cat => cat.id !== category.id);
  ...
  return (
    <div className={styles.container}>
                  ...
                  <div className={styles.deviceSelector}>
                    <Button
                      ...
                      style={{
                        width: '100%',
                        justifyContent: 'space-between',
                        backgroundColor: selectedDevice?.mode === 'fastboot' || selectedDevice?.mode === 'fastbootd' ? 'var(--colorNeutralBackground1)' : 'var(--colorNeutralBackground2)'
                      }}
                    >
                      {selectedDevice ? (
                      ...
                      <ChevronDown24Regular 
                        style={{ 
                          transform: isDeviceDropdownOpen ? 'rotate(180deg)' : 'rotate(0)',
                        ...
                        {devices.length === 0 ? (
                          ...
                          devices.map((device) => (
                            <div
                              ...
                              style={{ 
                                backgroundColor: selectedDevice?.serial === device.serial 
            ...
            <div className={styles.topControlsRow}>
              ...
              <div className={styles.commandInputRow}>
                <Field style={{ marginBottom: 0, width: '100%' }}>
                  <Field label={selectedDevice && (selectedDevice.mode === 'fastboot' || selectedDevice.mode === 'fastbootd') ? "Fastboot命令（注意：输入命令请谨慎，因输入错误命令导致的问题我们概不负责）" : "ADB命令（注意：输入命令请谨慎，因输入错误命令导致的问题我们概不负责）"}>
                     <Input
                       value={command}
                       onChange={(_, data) => setCommand(data.value)}
                       placeholder={selectedDevice && (selectedDevice.mode === 'fastboot' || selectedDevice.mode === 'fastbootd') 
                         ? "例如: devices" 
                    ...
                    {category.commands.map((cmd, cmdIndex) => (
                      ...
                      >
                        <div style={{ paddingRight: '8px' }}>
                          <div className={styles.commandLabel} title={cmd.label}>
                            {cmd.label}
                          </div>
                          <div className={styles.commandText} title={cmd.command}>
                            {cmd.command}
                          </div>
                          {cmd.description && (
                            <div className={styles.commandDescription} title={cmd.description}>
                              {cmd.description}
                            </div>
                          )}
                        </div>
      ...
      {/* 编辑命令对话框 */}
      <Dialog
        open={isEditDialogOpen && (isAddingNewCommand || isAddingNewCategory || !!editingCommand)}
        onOpenChange={(event, data) => {
                    ...
                    <Field label={t('command_panel.select_category')}>
                      <div style={{ display: 'flex', gap: '8px', marginBottom: '8px' }}>
                        {commandsConfig?.categories.map((cat) => (
                          <Button
                            key={cat.id}
                            appearance={editingCategory?.id === cat.id ? "primary" : "subtle"}
                            size="small"
